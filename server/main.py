...
def generate_sample_pitch(startup_info):
    ...
    return {
        "problem": {
            "text": f"Businesses struggle with finding reliable face recognition solutions that balance accuracy, privacy, and ethical considerations. Traditional systems have high error rates for diverse populations and raise significant security concerns.",
            "confidence": 0.85
        },
        "solution": {
            "text": f"{startup_name} provides advanced AI-powered face recognition technology that achieves 99.7% accuracy across all demographic groups while maintaining strong privacy safeguards and ethical compliance.",
            "confidence": 0.9
        },
        "market": {
            "text": f"The global facial recognition market is projected to reach $12.7B by 2028, growing at 16.8% CAGR. Our target segments in security, access control, and identity verification represent a $4.2B immediately addressable market.",
            "confidence": 0.88
        },
        "business_model": {
...
def improve_section_text(section_name, user_input):
    ...
    if not anthropic_api_key:
        ...
        return f"Based on your input about '{user_input[:30]}...', here's an improved version that incorporates your specific details while maintaining a compelling narrative structure for investors. The text highlights key strengths, addresses investor concerns, and presents a clear value proposition with supporting evidence."
...
Respond ONLY with the improved section text, nothing else."""
        ...
        payload = {
            ...
            "max_tokens": 1000,
            "temperature": 0.7,
            "system": "You are an expert startup pitch writer helping to create compelling pitch content for investors.",
        ...
        if response.status_code == 200:
            ...
            return f"Our {section_name} solution addresses the core challenges you mentioned about {user_input[:30]}... We've incorporated specific metrics and industry benchmarks to demonstrate our competitive advantage while maintaining the compelling narrative investors expect."
            
    except Exception as e:
        print(f"Exception calling Anthropic API: {str(e)}")
        return f"Our {section_name} solution addresses the core challenges in this space. Based on your feedback about {user_input[:30]}..., we've designed an approach that delivers significant value and addresses the key market needs. This provides our customers with measurable improvements in efficiency, accuracy, and cost-effectiveness."

# Function to generate mock email for investor
def generate_mock_email(investor, startup_name="AI Face Recognition"):
    email_template = f"""
Subject: Introducing {startup_name}: AI Face Recognition Technology Seeking {investor.get('investment_range', '$1M - $3M')} Investment

Dear {investor.get('name', 'Investor')},

I hope this email finds you well. My name is [Founder Name], founder of {startup_name}. I'm reaching out because our AI face recognition technology aligns perfectly with your investment focus on {', '.join(investor.get('sectors', ['AI', 'Technology']))}.

{startup_name} has developed advanced facial recognition technology that achieves 99.7% accuracy while addressing key market concerns around privacy and bias. Our solution is currently being used by 100+ clients across security, access control, and identity verification use cases.
...
We're raising {investor.get('investment_range', '$1M - $3M')} to scale our sales team and expand our enterprise client base. Given your investments in {', '.join(investor.get('portfolio', ['AI Companies']))}, I believe there could be strong synergies and would value your expertise in scaling B2B SaaS businesses.
